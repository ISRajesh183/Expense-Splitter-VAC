<template>
  <div class="p-6 max-w-3xl mx-auto bg-white shadow rounded-2xl mt-10">
    ...
    <button class="balance-btn" @click="goToBalancePage">
      💰 View My Balance
    </button>
  </div>
</template>
...
const createExpense = async () => {
  try {
    const payload = {
      ...
      paidBy: auth.user?.id || auth.user?.email, // required field
      participants: selectedPeople.value.map((id) => ({
        personId: id,
        shareAmount: parseFloat(splits.value[id] || equalShare.value),
      })),
      category: 'General', // optional, can make a select dropdown later
